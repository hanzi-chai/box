...
export function* simurate(mb: Mabiao, re: RegExp, map: Map<string, MbItem>, article: string, opt?: SimurateOptions) {
  ...
  const comboDist: number[][] = Array.from<number, number[]>({ length: 128 }, () => Array.from<number>({ length: 128 }).fill(0))
  /** 方案里缺少的字符, 无法打出的字符 */
  ...
  while (index < articleLength) {
    ...
    if (firstCharacter in cn) {
      // 破折号
      if (handleTwoCharacterPunctuations("——", cn["——"], SegKind.CnPunc))
        continue
      if (handleTwoCharacterPunctuations("……", cn["……"], SegKind.CnPunc))
        continue
      // 单个字符的中文标点
  ...
  for (let a = 0; a < comboDist.length; a++) {
    for (let b = 0; b < comboDist[a].length; b++) {
      ...
      // @ts-expect-error magic bit operator
