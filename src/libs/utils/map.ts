export function sumOfObjValues(usage: Record<any, number>) {
  let rs = 0
...
/** 把频数映射转换成频率映射 */
export function freqCountToFreq(map: Record<string, number>) {
  const total_freq = sumOfObjValues(map)
  const rs = { ...map }
...
/** 两个集合的交集 */
export function setsIntersection<T extends Set<string | number | bigint>>(
  setA: T,
  setB: T,
): T {
...
export function pickObject<T extends object>(
  obj: T,
  keys: Iterable<string | number | symbol>,
): T {
  const rs: any = {}
...
export function sumOfMapValues(aMap: Map<any, number>) {
  let sum = 0
