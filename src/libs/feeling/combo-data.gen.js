// TODO: 除了当量，其余的指标是互斥的，可以进一步减小尺寸
import { writeFileSync } from "node:fs"
import { KEYS_EQ } from "@/libs/constants"
import { ComboType } from "./combo-types.ts"

export const KEYS = [...KEYS_EQ]
...
console.log("maxDl is %d", Math.max(...dlMap.values())) // 26

const keysUnderFinger = [
  "1qaz",
  "2wsx",
  "3edc",
  "4rfv5tgb",
...
const longFingerDisturb = [
  ...
  res.push(a[0] + c[1], a[1] + c[0])
  if (c.length > 2) {
    res.push(a[0] + c[3], a[1] + c[2])
  }
  return res
...
const pinkyDisturb = [
  ["1qaz", "2wsx3edc"],
  ["0p;/-['=]", "8ik,9ol."],
].map(([little, long]) => [...little].map((k) => [...long].map((k2) => k + k2)))
...
const pdSet = makeSet(pinkyDisturb)

// const left = new Set("12345qwertasdfgzxcvb")
// const right = new Set(String.raw`67890-=yuiop[]\hjkl;'nm,./`)

const result = KEYS.map((v1) => KEYS.map((v2) => keyPairToMagic(v1 + v2)))
const resultJson = JSON.stringify(result.flat())

writeFileSync("combo-data.js", `export default "${resultJson}"`)

// 209
console.log("max magic number is %d", Math.max(...result.flat()))
...
function keyPairToMagic(pair) {
  const result = Math.trunc(Number.parseInt(dlMap.get(pair) ?? 0)) << 3
  // 当量 空间是 ~0b111，当量的最大值是 26
  if (msSet.has(pair)) return result | ComboType.MultiSpan
  if (ssSet.has(pair)) return result | ComboType.SingleSpan
  if (lfdSet.has(pair)) return result | ComboType.LongFingersDisturb
  if (pdSet.has(pair)) return result | ComboType.PinkyDisturb
  if (pair[0] === pair[1]) return result | ComboType.DoubleHit
  if (LEFT_SET.has(pair[0]) ^ LEFT_SET.has(pair[1]))
    return result | ComboType.DifferentHands
  return result
}
...
function keyPairToMagic(pair) {
    return (
        ...
        ((pdSet.has(pair) | 0) << 5) | // 小指干扰
        // | ((isDifferentHand(pair) | 0) << 5) // 左右互击
        Math.trunc(Number.parseInt(dlMap.get(pair) ?? 0)) // 当量 空间是 0 ~ 0x1F
    )
}*/
