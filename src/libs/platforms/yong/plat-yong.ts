...
function load(raw: string, title?: string): TextMabiao {
  ...
  if (dataIndex !== -1) {
    ...
    for (const [line, lineno] of utils.genEachLineJump(header)) {
      headerLines = lineno
      // 跳过注释
      if (utils.quickStartWith(line, "#")) continue
  ...
  for (const [line, lineno] of utils.genEachLineJump(
    ...
    if (!validateCodes(cd)) throw new FormatError("编码含有非法字符", lineno)

    if (words.length === 0) throw new FormatError("没有词语", lineno)
...
function validate(raw: string): boolean {
  // 有[data]标记，则肯定是小小的格式
  const dataIndex = utils.indexOfLineEqualsIgnoreCase(raw, "[data]")
  if (dataIndex !== -1) return true

  // 检查码表数据行的格式
  for (const [line, lineno] of utils.genEachLineJump(raw)) {
    if (lineno > 20) return true
    // 检查每一行是 编码 空格 词组
    const [code, ...words] = utils.quickSplit(line, " ")
    if (validateCodes(code) && words.length > 0) continue
    return false
  }
  return true
}
